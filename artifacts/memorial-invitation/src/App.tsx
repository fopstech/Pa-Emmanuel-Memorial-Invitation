...
function AsoEbiCountdown() {
  const target = useMemo(() => new Date("2026-10-01T00:00:00+01:00"), []);
  const { days, hours, minutes, seconds, isPast } = useCountdown(target);
  return <p className="mt-5 rounded-xl border border-[#e1b47d]/20 bg-background/10 px-4 py-3 text-sm text-background/75">{isPast ? <strong className="text-[#e1b47d]">Aso-Ebi sales are now closed.</strong> : <>Sales close in <strong className="text-[#e1b47d]">{days}d {hours}h {minutes}m {seconds}s</strong> · 30 September 2026, WAT.</>}</p>;
}

function AsoEbiSection({ information }: { information?: string | null }) {
  const lines = (information || defaultAsoEbi).split("\\n");
  return <><section id="aso-ebi" className="bg-[#252a3a] px-5 py-20 text-background lg:px-10 lg:py-28" data-testid="section-aso-ebi"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div className="relative min-h-[300px] overflow-hidden rounded-[2rem] border border-[#e1b47d]/25 bg-[#352448] p-8 shadow-2xl"><div className="absolute -right-10 -top-14 h-56 w-56 rounded-full border-[28px] border-[#e1b47d]/15" /><div className="absolute -bottom-16 -left-12 h-48 w-48 rounded-full border-[20px] border-[#b186cb]/20" /><div className="relative flex h-full min-h-[235px] flex-col justify-between"><p className="mono-font text-[10px] uppercase tracking-[.18em] text-[#e1b47d]">Aso-Ebi · family cloth</p><p className="display-font max-w-xs text-5xl leading-none">A thoughtful way to stand together.</p><p className="text-sm text-background/60">Available to family and friends who would like to join the remembrance.</p></div></div><div><p className="mono-font text-xs uppercase text-[#e1b47d]">04 · Aso-Ebi</p><h2 className="display-font mt-5 text-4xl leading-tight sm:text-6xl">Wear a little of the memory.</h2><AsoEbiCountdown /><div className="mt-8 space-y-1 text-sm leading-7 text-background/70">{lines.map((line, index) => line ? <p key={`${line}-${index}`} className={index === 0 ? "font-bold uppercase tracking-[.12em] text-[#e1b47d]" : ""}>{line}</p> : <div key={`space-${index}`} className="h-3" />)}</div></div></div></section><LocationMap /></>;
}
...
function GuestManager({ mode }: { mode: "guests" | "invitations" }) {
  ...
  const importGuests = () => {
    if (!importConfirmed) {
      ...
      toast({ title: "Review the preview", description: "Press Import guests again to confirm creating these invitations." });
      return;
    }
    importer.mutate({ data: { csv } }, { onSuccess: result => { setImportResult({ created: result.created.length, errors: result.errors.length, duplicates: result.duplicates.length }); setImportConfirmed(false); refresh(); toast({ title: "Guest list imported" }); }, onError: () => toast({ title: "Import failed", description: "Please review the pasted rows and try again." }) });
  };
