...
import {
  ...
  useLookupPublicInvitation, useRegenerateInvitation, useRequestUploadUrl, useSubmitRsvp,
...
function GuestRow({ guest, onEdit, onDelete, onToggle, onRegenerate }: { guest: AdminGuest; onEdit: () => void; onDelete: () => void; onToggle: () => void; onRegenerate: () => void }) {
  ...
  return <article className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between" data-testid={`row-guest-${guest.id}`}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-bold">{guest.fullName}</p>{isTest && <StatusPill>Development test</StatusPill>}</div><p className="mt-1 text-xs text-muted-foreground">{guest.email || guest.phone || "No contact details"}</p><div className="mt-3 flex flex-wrap items-center gap-2"><StatusPill tone={guest.rsvpStatus === "yes" ? "good" : guest.rsvpStatus === "no" ? "bad" : "warm"}>{guest.rsvpStatus}</StatusPill>{guest.checkedInAt ? <StatusPill tone="good">Checked in</StatusPill> : <StatusPill>Not checked in</StatusPill>}<span className="mono-font text-xs text-muted-foreground">{guest.invitationCode}</span><span className="text-xs text-muted-foreground">· {guest.admittedCount}/{guest.admissionLimit} admitted</span></div></div><div className="flex flex-wrap justify-start gap-1 sm:max-w-[420px] sm:justify-end"><a href={link} target="_blank" rel="noreferrer" className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-lg px-3 text-xs font-bold text-accent hover:bg-muted"><ExternalLink className="h-4 w-4" /> View</a><button className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-lg px-3 text-xs font-bold text-muted-foreground hover:bg-muted" onClick={() => copyText(link, "Invitation link")}><Copy className="h-4 w-4" /> Link</button><button className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-lg px-3 text-xs font-bold text-muted-foreground hover:bg-muted" onClick={() => copyText(guest.invitationCode, "Invitation code")}><Copy className="h-4 w-4" /> Code</button><button className="focus-ring rounded-lg p-2 text-muted-foreground hover:bg-muted" title="Edit guest" onClick={onEdit}><Pencil className="h-4 w-4" /></button><button className="focus-ring rounded-lg p-2 text-muted-foreground hover:bg-muted" title="Regenerate invitation" onClick={onRegenerate}><RefreshCw className="h-4 w-4" /></button><button className="focus-ring rounded-lg p-2 text-muted-foreground hover:bg-muted" title={guest.status === "disabled" ? "Enable invitation" : "Disable invitation"} onClick={onToggle}>{guest.status === "disabled" ? <Check className="h-4 w-4" /> : <X className="h-4 w-4" />}</button><button className="focus-ring rounded-lg p-2 text-muted-foreground hover:bg-[#f4dfdc] hover:text-destructive" title="Delete guest" onClick={onDelete}><Trash2 className="h-4 w-4" /></button></div></article>;
}
